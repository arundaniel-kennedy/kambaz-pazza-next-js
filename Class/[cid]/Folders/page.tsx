...
export default function FilterBar() {
  ...
  const selectedIndex = useSelector((state: RootState) => state.filter.selectedIndex);
  const dispatch = useDispatch();

  return (
    <div id="pazza-main" className="flex items-center gap-3 bg-[#1a4d7a] overflow-x-auto">
      {items.map((item: any, index: number) => (
        ...
        >
          ...
          {item.count !== undefined && (
            <span className={`rounded-full text-xs p-1 text-center ${
              selectedIndex === index 
